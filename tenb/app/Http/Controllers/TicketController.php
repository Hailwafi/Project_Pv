    ...
    public function store(Request $request)
{
    ...
    if ($ticket) 
    {
        ...
        return response()->json([
            ...
            'redirect_url' => url('/tickets/' . $ticket->id),
        ], 201);
    }

    return new TicketResource(false, 'Gagal membuat Ticket!', null);
}
    ...
    public function assignTicket(Request $request, $id)
    {
        ...
        // Update tiket pegawai dengan staf yang ditugaskan
        $ticket->assigned_to = $request->assigned_to;
        $ticket->save();

        // Kirim notifikasi ke staf yang ditugaskan
        $staf = User::find($request->assigned_to);
        $staf->notify(new TicketAssignedNotification($ticket, 'pegawai'));  // 'pegawai' tipe tiket

        return response()->json(['message' => 'Tiket Pegawai berhasil ditugaskan kepada Staf.'], 200);
    }
}
